...
const Logout = asyncHandler(async (req, res) => {
  res.cookie("token", "", {
    path: "/",
    httpOnly: true,
    expires: new Date(0), // set to current second to expire the cookie and logout the user
    sameSite: "none",
    secure: true,
  });
...
const forgotPassword = asyncHandler(async (req, res) => {
  ...
  // Find user
  const forgotUser = await User.findOne({ _id: userToken.userId });
